...
class SCTNet_B(nn.Module):
    def __init__(self,num_classes=19
                 ):
        super(SCTNet_B,self).__init__()
        self.backbone = SCTNet()
        self.decoder = SCTHead(
            in_channels=64*4,
            channels=128*2,
            dropout_ratio=0.0,
            in_index=0,
...
class SCTNet_S(nn.Module):
    def __init__(self,num_classes=19
                 ):
        super(SCTNet_S,self).__init__()
        self.backbone = SCTNet(
            base_channels=32,  #Slim32  
            spp_channels=64, 
            )
        self.decoder = SCTHead(
            in_channels=32*4,
